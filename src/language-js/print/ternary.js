...
function printTernary(path, options, print) {
  ...
  if (
    ...
    const part = [
      ...
      ...(consequentNode.type === node.type
        ? [ifBreak("", "("), parenSpace]
        : [""]),
      // [prettierx merge update(s) from prettier@2.3.1 ...] offsetTernaryExpressions option support:
    ...
    parts.push(
      // [prettierx merge update(s) from prettier@2.3.1 ...] with offsetTernaryExpressions option support below:
      parent.type !== node.type ||
        parent[alternateNodePropertyName] === node ||
        isParentTest
        ? part
        : options.useTabs || options.offsetTernaryExpressions // [prettierx merge update(s) from prettier@2.3.1 ...] offsetTernaryExpressions option support (...)
        ? dedent(indent(part))
  ...
  // [prettierx merge update(s) from prettier@2.3.1 ...] moved & updated for --space-in-parens option support (...)
  ...
  // prettierx merge update(s) from prettier@2.3.1 ...: with options for parenSpace support in ternaries (...)
  const maybeGroup = (doc) =>
  ...
  const result = maybeGroup([
    ...
    // [prettierx merge update(s) from prettier@2.3.1 ...]: spaceInParens option (...)
