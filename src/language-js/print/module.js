...
function printModuleSpecifiers(path, options, print) {
  ...
  if (isNonEmptyArray(node.specifiers)) {
    ...
    // [prettierx merge update(s) from prettier@2.3.1 ...] for exportCurlySpacing, import CurlySpacing option support
    let isExport = false;

    // [prettierx merge update(s) from prettier@2.3.1 ...] with exportCurlySpacing, import CurlySpacing option support
    path.each(() => {
      ...
      if (
        ...
        // [prettierx merge update(s) from prettier@2.3.1 ...] exportCurlySpacing option
        isExport = true;
        ...
        // [prettierx merge update(s) from prettier@2.3.1 ...] importCurlySpacing option
        isExport = false;
        standaloneSpecifiers.push(print());
    ...
    if (groupedSpecifiers.length > 0) {
      ...
      const canBreak =
        // prettierx merge update(s) from prettier@2.3.1 ...: importFormatting
      ...
      if (canBreak) {
        parts.push(
          group([
            "{",
            indent([
              // [prettierx merge update(s) from prettier@2.3.1 ...] importCurlySpacing, exportCurlySpacing options
              curlyLine,
